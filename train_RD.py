# -*- coding: utf-8 -*-
"""Generic training script for RD-family segmentation models.

适配对象：RD_v1 / RD_v2 / 后续 RD_v3... 这类返回 dict 的模型，也兼容普通 tensor 输出模型。
核心特性：
...
def collect_output_stats(outputs, masks=None, threshold: float = 0.5) -> Dict[str, float]:
    ...
    for k, v in outputs.items():
        lk = k.lower()
        if torch.is_tensor(v) and v.numel() <= 16 and any(s in lk for s in ("alpha", "gate", "weight")):
            if v.dim() <= 2:
                stats[k] = tensor_to_float(v)
        if isinstance(v, dict):
            for kk, vv in v.items():
                lkk = kk.lower()
                if torch.is_tensor(vv) and vv.numel() <= 16 and any(s in lkk for s in ("alpha", "gate", "weight")):
...
def main():
    ...
    # 记录完整实验时间，和普通 train.py 的日志风格保持一致。
    ...
    if args.resume:
        ...
        if isinstance(ckpt, dict):
            ...
            saved_metrics = ckpt.get("metrics", {}) if isinstance(ckpt.get("metrics", {}), dict) else {}
            best_iou = float(saved_metrics.get("best_iou", best_iou))
            best_epoch = int(saved_metrics.get("best_epoch", best_epoch))
            best_ema_iou = float(saved_metrics.get("best_ema_iou", best_ema_iou))
            best_ema_epoch = int(saved_metrics.get("best_ema_epoch", best_ema_epoch))
            no_improve = int(saved_metrics.get("no_improve", saved_metrics.get("epochs_without_improvement", no_improve)))
        log_print(f"Resume from {args.resume}, start_epoch={start_epoch}, no_improve={no_improve}\n", log_file)
    ...
    hyper_msg = (
        f"--------------------------------------------------\n"
        f" 训练超参数设置:\n"
        f" - 主学习率配置 lr: {float(train_cfg.get('lr', 1e-4))}\n"
        f" - 权重衰减 (Weight Decay): {float(train_cfg.get('weight_decay', 1e-2))}\n"
        f" - 学习率调度 (LR Scheduler): ReduceLROnPlateau "
        f"(factor={float(train_cfg.get('lr_factor', 0.5))}, patience={int(train_cfg.get('lr_patience', 5))})\n"
        f" - 早停机制 (Early Stopping Patience): {early_stop} 轮\n"
        f"--------------------------------------------------\n"
    )
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if train_grad_stats:
            ...
            detail_show = [k for k in detail_keys if ("dino_adapters" in k or "dlaem" in k or "cnn_encoder" in k or "decoder" in k)][:24]
            if detail_show:
        ...
        # 先判断本轮是否刷新 raw IoU，保证日志和 metrics.csv 中的 no_improve 都是当前轮之后的真实值。
        improved = raw["iou"] > best_iou
        current_no_improve = 0 if improved else no_improve + 1
        best_iou_so_far = raw["iou"] if improved else best_iou
        ...
        if improved:
            ...
            save_metrics["no_improve"] = current_no_improve
        ...
        if ema_metrics is not None and ema_metrics["iou"] > best_ema_iou:
            ...
            save_ema_metrics["no_improve"] = current_no_improve
            torch.save({
                "epoch": epoch,
        ...
        last_metrics["no_improve"] = current_no_improve
        last_metrics["early_stop_patience"] = early_stop
        ...
        # 和普通 train.py 一样，每轮明确记录“连续多少轮没有提升”。
        no_improve = current_no_improve
        if improved:
            improve_msg = f"      已保存最佳权重！当前 raw IoU={best_iou*100:.2f}%\n"
            log_print(improve_msg, log_file)
        else:
            no_improve_msg = f"     ⚠️ 连续 {no_improve} 轮没有提升了。当前最佳 raw IoU={best_iou*100:.2f}% / epoch={best_epoch}\n"
            log_print(no_improve_msg, log_file)

        if no_improve >= early_stop:
            log_print(f"🚫 连续 {early_stop} 轮 IoU 未提升，触发早停机制，训练提前结束！\n", log_file)
    ...
    # 训练彻底结束后的时间统计，和普通 train.py 保持一致。
    ...
    end_msg = (
        ...
        f" 最佳 raw IoU: {best_iou*100:.2f}% @ epoch={best_epoch}\n"
        f" 最佳 EMA IoU: {best_ema_iou*100:.2f}% @ epoch={best_ema_epoch}\n"
