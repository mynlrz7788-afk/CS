...
def main():
    ...
    model_cfg_msg = (
        ...
        f" - WPDA层: {config['model'].get('wpda_layers', 'N/A')}\n"
        f" - WPDA window/prototypes: {config['model'].get('wpda_window_size', 'N/A')} / {config['model'].get('wpda_num_prototypes', 'N/A')}\n"
        f" - WPDA dropout: {config['model'].get('wpda_dropout', 'N/A')}\n"
        f"--------------------------------------------------\n"
    )
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for batch_data in train_loader_tqdm:
            ...
            if grad_norm is None:
                ...
                train_loader_tqdm.set_postfix({"loss": f"{loss.item():.4f}", "grad": f"{grad_norm:.2f}"})
