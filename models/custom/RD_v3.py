# -*- coding: utf-8 -*-
"""
RD_v3: Standalone Baseline-stable CNN + PEFT-DINO Road-aware DLAEM.

本文件已经内置 RD_v2 中 RD_v3 需要的全部 DINO / DLAEM / LightGate 模块，
...
class RD_v3(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(self.dino_layers) != 4:
            raise ValueError(f"RD_v3 需要 4 个 DINO 层，例如 [2,5,8,11]，当前: {self.dino_layers}")
    ...
    def _print_model_info(self):
        ...
        print("-" * 60)
        print("[RD_v3] Standalone Baseline-stable CNN + PEFT-DINO Road-aware DLAEM")
        print(f"    - Total params:     {total / 1e6:.2f} M")
        print(f"    - Trainable params: {trainable / 1e6:.2f} M")
        print(f"    - Frozen params:    {frozen / 1e6:.2f} M")
        print(f"    - DINO total/train: {dino_total / 1e6:.2f} M / {dino_train / 1e6:.2f} M")
        print("    - Data flow: X -> PEFT-DINO token bank + HL_base encoder -> DLAEM8 -> layer3 -> DLAEM16 -> layer4 -> HL_base decoder")
...
if __name__ == "__main__":
    ...
    print("RD_v3 module loaded. Instantiate through models.get_model(config['model']).")
