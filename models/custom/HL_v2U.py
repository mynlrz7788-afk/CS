...
class AuxSegHead(nn.Module):
    """
    Auxiliary Segmentation Head（辅助分割头）

    用于真正的 decoder deep supervision（解码器深监督）：
    - d1: H/4,  96 channels，细节/边界层，权重建议 0.3；
    - d2: H/8,  128 channels，局部结构层，权重建议 0.2；
    - d3: H/16, 256 channels，深层语义层，权重建议 0.1。

    注意：辅助 logits 会上采样到原图大小后再与原始 mask 计算 loss，
    避免下采样 mask 导致细道路消失。
    """
    def __init__(self, in_channels, mid_channels=64, num_classes=1):
        super().__init__()
        mid_channels = max(mid_channels, num_classes)
...
class HL_v2U(nn.Module):
    """
    HL_v2U / R-MHRNet:
    Road-aware Mutual-guided High-low Routing Network
    道路感知相互指导高低分辨率路由网络

    和 HL_v1 的主要区别：
    1) 不再维护重型 high-resolution branch；
    2) 在关键 skip connection 处引入 MHR-MoE；
    3) 先高低分辨率相互指导，再进行道路专家路由；
    4) 使用 Frequency Detail Expert + Strip Direction Expert + Context/Suppression Expert；
    5) 输出头在 H/2 上预测，再上采样到原图，避免 HL_v1 的全分辨率重型双头。
    """
    def __init__(
        self,
        n_channels=3,
        ...
        **kwargs,
        ...
        # Lightweight H/2 output head（主输出头）
        ...
        # True decoder deep supervision heads（真正的解码器深监督头）
        # 训练时 return_aux=True 才会计算；测试时 return_aux=False，不增加推理开销。
        self.aux_head_d1 = AuxSegHead(in_channels=96, mid_channels=64, num_classes=n_classes)    # H/4
        self.aux_head_d2 = AuxSegHead(in_channels=128, mid_channels=64, num_classes=n_classes)   # H/8
        self.aux_head_d3 = AuxSegHead(in_channels=256, mid_channels=64, num_classes=n_classes)   # H/16
    ...
    def forward_features(self, x):
        ...
        # 只有训练时需要辅助监督时才计算辅助头，测试/复杂度统计时不增加额外推理开销。
        if self.return_aux:
            aux["aux_logits_d1"] = self.aux_head_d1(d1, out_size=input_size)  # H/4 -> H
            aux["aux_logits_d2"] = self.aux_head_d2(d2, out_size=input_size)  # H/8 -> H
            aux["aux_logits_d3"] = self.aux_head_d3(d3, out_size=input_size)  # H/16 -> H
...
if __name__ == "__main__":
    model = HL_v2U(n_channels=3, n_classes=1, pretrained=False, return_aux=False)
