# -*- coding: utf-8 -*-
"""
RD_v1_A: PEFT-DINO-only single-layer pseudo pyramid + RD decoder.

本文件是 RD_v1_A 的单层 DINO 消融版：
...
5) 保留 RD_v4 的 HL_base-style residual decoder、out head 和 aux heads。
"""
...
class DINOMapFuse(nn.Module):
    ...
    本模块只做尺度适配和通道适配，不引入 RGB/CNN stem，保证 RD_v1_A 是干净的 DINO-only 消融。
    """
...
class DINOSingleLayerPseudoPyramid(nn.Module):
    """Build d0/d1/d2/d3/d4 from single DINO layers.
...
# =========================================================
# 4. RD_v1_A main model
# =========================================================
class RD_v1_A(nn.Module):
    """RD_v1_A: PEFT-DINO-only single-layer pseudo pyramid + RD_v4 decoder.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if int(n_channels) != 3:
            raise ValueError("RD_v1_A 是单层 DINO-only 消融版，当前只支持 3 通道输入。")
        ...
        if len(self.dino_layers) != 4:
            raise ValueError(f"RD_v1_A 需要 4 个 DINO 层，例如 [2,5,8,11]，当前: {self.dino_layers}")
    ...
    def get_param_groups(self, base_lr: float = 1e-4, weight_decay: float = 1e-2):
        ...
        RD_v1_A 没有 cnn_encoder / road_priors / dlaem 参数组。
        """
    ...
    def _print_model_info(self):
        ...
        print("-" * 60)
        print("[RD_v1_A] PEFT-DINO-only Single-Layer Pseudo Pyramid + RD Decoder")
        ...
        print("    - Data flow: X -> PEFT-DINO A2/A5/A8/A11 -> A11:d4, A8:d3, A5:d2, A2:d1/d0 -> RD decoder")
        print("-" * 60)


if __name__ == "__main__":
    print("RD_v1_A module loaded. Instantiate through models.get_model(config['model']).")
