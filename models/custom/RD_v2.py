# -*- coding: utf-8 -*-
"""
RD_v2: Truncated Baseline-stable CNN + PEFT-DINO Road-aware DLAEM with DINO-Guided Center16.

本文件内置 RD_v2 需要的全部 DINO / DLAEM / LightGate / DINO-Guided Center16 模块，
以及 HL_base 中的 residual decoder 结构；部署时不再依赖 RD_v2.py 或 HL_base.py。

重要修复：
1) DLAEM8/DLAEM16 的 alpha 不再从 0 初始化，而是默认 0.03，避免 DLAEM 梯度被关死；
2) LightGate4 的 alpha 默认 0.02；
...
def build_dinov3_backbone(model_name: str, ckpt_path: str, dino_repo_path: str):
    ...
    if ckpt_path:
        ...
        print(
            f"[RD_v2] 加载 DINOv3 权重完成: {ckpt_path}\n"
...
class PEFTDINOv3GlobalBranch(nn.Module):
    ...
    def __init__(
        ...
        print(
            f"[RD_v2] PEFT-DINOv3 ready. layers={self.out_layers}, patch={self.patch_size}, "
...
class RoadPriorHead(nn.Module):
    ...
    RD_v2 keeps the prior branch, but avoids an expensive full 3x3 conv at C16=512.
...
class RoadAwareDLAEM(nn.Module):
    ...
    RD_v2 decouples external CNN feature width and internal DLAEM width.
...
class LightLAEMGate4(nn.Module):
    """C4 的轻量语义门控，不做 1/4 deformable attention。

    RD_v2 uses a small internal semantic dimension instead of projecting DINO tokens
    directly to all C4 channels.
    """
    def __init__(self, c4_dim: int = 128, dino_dim: int = 384, inner_dim: int = 64, gn_groups: int = 8):
        super().__init__()
        inner_dim = int(inner_dim)
        self.proj_a = nn.Linear(dino_dim, inner_dim)
        self.proj_b = nn.Linear(dino_dim, inner_dim)
        self.semantic_refine = nn.Sequential(
            ConvGNAct(inner_dim, inner_dim, 3, gn_groups=gn_groups),
            DepthwiseSeparableConv(inner_dim, inner_dim, 3, gn_groups=gn_groups),
            ECALayer(inner_dim),
        )
        self.semantic_out = nn.Sequential(
            nn.Conv2d(inner_dim, c4_dim, 1, bias=False),
            _make_gn(c4_dim, gn_groups),
            nn.GELU(),
        )
        self.gate = nn.Sequential(
            ConvGNAct(c4_dim * 2 + 1, inner_dim, 1, gn_groups=gn_groups),
            DepthwiseSeparableConv(inner_dim, inner_dim, 3, gn_groups=gn_groups),
            nn.Conv2d(inner_dim, c4_dim, 1),
            nn.Sigmoid(),
        )
        self.alpha = BoundedScalar(max_value=0.45, init_value=0.02)

    def _tokens_to_map(self, token, dino_hw, proj):
        h, w = int(dino_hw[0]), int(dino_hw[1])
        b, n, _ = token.shape
        if n != h * w:
            raise RuntimeError(f"LightGate4 token 数与 dino_hw 不匹配: token={n}, hw={h}x{w}")
        x = proj(token).transpose(1, 2).contiguous().view(b, -1, h, w)
        return x

    def forward(self, c4, prior_prob, tokens_a, tokens_b, dino_hw):
        sa = self._tokens_to_map(tokens_a, dino_hw, self.proj_a)
        sb = self._tokens_to_map(tokens_b, dino_hw, self.proj_b)
        s = 0.5 * (sa + sb)
        s = F.interpolate(s, size=c4.shape[-2:], mode="bilinear", align_corners=False)
        s = self.semantic_refine(s)
        s = self.semantic_out(s)
        if prior_prob.shape[-2:] != c4.shape[-2:]:
            prior_prob = F.interpolate(prior_prob, size=c4.shape[-2:], mode="bilinear", align_corners=False)
        g = self.gate(torch.cat([c4, s, prior_prob], dim=1))
        return c4 + self.alpha() * g * s
...
class CenterLocalRoadRefine(nn.Module):
    """1/16 decoder 起点的局部道路结构精修。

    目的：替代删除 x4/layer4 后损失的部分深层上下文，同时保留道路细长结构。
    包含局部卷积、膨胀上下文和条带方向三类分支。
    """
    def __init__(self, channels: int = 256, gn_groups: int = 8, strip_kernel: int = 7):
        super().__init__()
        k = int(strip_kernel)
        if k % 2 == 0:
            k += 1
        pad = k // 2
        c = int(channels)
        self.local = DepthwiseSeparableConv(c, c, 3, dilation=1, gn_groups=gn_groups)
        self.dilated2 = DepthwiseSeparableConv(c, c, 3, dilation=2, gn_groups=gn_groups)
        self.dilated3 = DepthwiseSeparableConv(c, c, 3, dilation=3, gn_groups=gn_groups)
        ...
        self.fuse = nn.Sequential(
            ConvGNAct(c * 4, c, 1, gn_groups=gn_groups),
            DepthwiseSeparableConv(c, c, 3, gn_groups=gn_groups, act=False),
            ECALayer(c),
        )
        self.act = nn.GELU()
...
class DINOGuidedCenter16(nn.Module):
    """DINO-Guided Center16：用 A8/A11 替代 x4 的全局语义瓶颈。

    输入：
        x3_enh:  B,C, H/16,W/16，来自 DLAEM16 的 CNN-DINO 增强特征；
        prior16: B,1, H/16,W/16，道路软先验；
        token_a/token_b: A8/A11，B,N,384；
        dino_hw: DINO token map 的空间尺寸。
    输出：
        d3: B,C,H/16,W/16，作为 decoder 的最深起点。
    """
    def __init__(
        self,
        c_dim: int = 256,
        ...
        self.c_dim = int(c_dim)
        self.inner_dim = int(inner_dim)
        self.proj_a = nn.Linear(int(dino_dim), self.inner_dim)
        self.proj_b = nn.Linear(int(dino_dim), self.inner_dim)
        ...
        self.dino_fuse = nn.Sequential(
            ConvGNAct(self.inner_dim * 2, self.inner_dim, 1, gn_groups=gn_groups),
            DepthwiseSeparableConv(self.inner_dim, self.inner_dim, 3, gn_groups=gn_groups),
            ECALayer(self.inner_dim),
        )
        if self.inner_dim != self.c_dim:
            self.dino_out = nn.Sequential(
                nn.Conv2d(self.inner_dim, self.c_dim, 1, bias=False),
                _make_gn(self.c_dim, gn_groups),
                nn.GELU(),
            )
        else:
            self.dino_out = nn.Identity()

        gate_dim = int(gate_dim)
        self.gate = nn.Sequential(
            ConvGNAct(self.c_dim * 2 + 1, gate_dim, 1, gn_groups=gn_groups),
            DepthwiseSeparableConv(gate_dim, gate_dim, 3, gn_groups=gn_groups),
            nn.Conv2d(gate_dim, self.c_dim, 1),
            nn.Sigmoid(),
        )
        self.alpha = BoundedScalar(max_value=float(alpha_max), init_value=float(alpha_init))
        self.refine = CenterLocalRoadRefine(self.c_dim, gn_groups=gn_groups, strip_kernel=strip_kernel)

    def _tokens_to_map(self, token: torch.Tensor, dino_hw: Tuple[int, int], proj: nn.Linear) -> torch.Tensor:
        h, w = int(dino_hw[0]), int(dino_hw[1])
        b, n, _ = token.shape
        if n != h * w:
            raise RuntimeError(f"DINOGuidedCenter16 token 数与 dino_hw 不匹配: token={n}, hw={h}x{w}")
        return proj(token).transpose(1, 2).contiguous().view(b, self.inner_dim, h, w)

    def forward(
        self,
        x3_enh: torch.Tensor,
        prior16: torch.Tensor,
        token_a: torch.Tensor,
        token_b: torch.Tensor,
        dino_hw: Tuple[int, int],
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        da = self._tokens_to_map(token_a, dino_hw, self.proj_a)
        db = self._tokens_to_map(token_b, dino_hw, self.proj_b)
        dc = self.dino_fuse(torch.cat([da, db], dim=1))
        dc = self.dino_out(dc)
        if dc.shape[-2:] != x3_enh.shape[-2:]:
            dc = F.interpolate(dc, size=x3_enh.shape[-2:], mode="bilinear", align_corners=False)
        if prior16.shape[-2:] != x3_enh.shape[-2:]:
            prior16 = F.interpolate(prior16, size=x3_enh.shape[-2:], mode="bilinear", align_corners=False)
        gate = self.gate(torch.cat([x3_enh, dc, prior16], dim=1))
        fused = x3_enh + self.alpha() * gate * dc
        d3 = self.refine(fused)
        aux = {
            "center_alpha": self.alpha(),
            "center_gate_mean": gate.mean(),
            "center_dino_mean": dc.mean(),
        }
        return d3, aux
...
class ResNet34BaselineEncoder(nn.Module):
    ...
    尺度：
        ...
        x2: H/8,  128
        x3: H/16, 256
        RD_v2 删除 x4/layer4，不再产生 H/32 特征。
    """
    ...
    def __init__(self, in_channels: int = 3, pretrained: bool = True):
        ...
        # RD_v2 截断 ResNet34：删除 layer4/x4，避免 1/32 CNN bottleneck 过拟合。
...
class RD_v2(nn.Module):
    ...
    def __init__(
        ...
        dlaem8_dim: int = 256,
        dlaem16_dim: int = 256,
        light_gate_dim: int = 96,
        prior_mid_max: int = 128,
        aux_mid_ch: int = 64,
        center_dim: int = 256,
        center_gate_dim: int = 128,
        center_alpha_init: float = 0.03,
        center_alpha_max: float = 0.60,
        center_strip_kernel: int = 7,
        ...
        **kwargs,
        ...
        if len(self.dino_layers) != 4:
            raise ValueError(f"RD_v2 需要 4 个 DINO 层，例如 [2,5,8,11]，当前: {self.dino_layers}")
        ...
        # 固定 ResNet34 truncated baseline 通道：RD_v2 删除 c32/x4。
        ...
        self.light_gate4 = LightLAEMGate4(
            c4_dim=c4,
            dino_dim=int(dino_embed_dim),
            inner_dim=int(light_gate_dim),
            gn_groups=int(gn_groups),
        )

        # 5) RD_v2 decoder：删除 layer4/x4/dec3，新增 DINO-Guided Center16 作为 1/16 decoder 起点。
        self.center16 = DINOGuidedCenter16(
            c_dim=c16,
            dino_dim=int(dino_embed_dim),
            inner_dim=int(center_dim),
            gate_dim=int(center_gate_dim),
            alpha_init=float(center_alpha_init),
            alpha_max=float(center_alpha_max),
            gn_groups=int(gn_groups),
            strip_kernel=int(center_strip_kernel),
        )
    ...
    def forward_features(self, x: torch.Tensor) -> Dict[str, Any]:
        ...
        # 4) DLAEM8: x2 + P8 查询 A5/A8；x2' 串联进入 layer3。
        ...
        # 6) DLAEM16: x3 + P16 查询 A8/A11，得到 x3'。
        x3_enh, aux16 = self.dlaem16(x3, p16, [a8, a11], dino_hw)

        # 7) LightGate4: x1 + P4 + A2/A5，得到 x1'。
        x1_enh = self.light_gate4(x1, p4, a2, a5, dino_hw)

        # 8) RD_v2 删除 layer4/x4：DINO-Guided Center16 直接生成 1/16 decoder 起点 d3。
        d3, aux_center = self.center16(x3_enh, p16, a8, a11, dino_hw)  # H/16, 256

        # 9) Truncated decoder，skip 使用增强后的 x2'/x1' 和浅层 x0。
        ...
        logit16 = self.aux16_head(d3)            # H/16, decoder bottleneck supervision
        ...
        outputs: Dict[str, Any] = {
            ...
            "center16_alpha": aux_center.get("center_alpha"),
            "center16_gate_mean": aux_center.get("center_gate_mean"),
            "center16_dino_mean": aux_center.get("center_dino_mean"),
    ...
    def get_param_groups(self, base_lr: float = 1e-4, weight_decay: float = 1e-2):
        ...
        add_group("dlaem", list(self.dlaem8.parameters()) + list(self.dlaem16.parameters()) + list(self.light_gate4.parameters()), default_lr_mult=1.00)
        add_group("center", self.center16.parameters(), default_lr_mult=1.10)
        decoder_params = (
            list(self.dec2.parameters()) + list(self.dec1.parameters()) + list(self.dec0.parameters())
    ...
    def _print_model_info(self):
        ...
        print("-" * 60)
        print("[RD_v2] Truncated CNN + PEFT-DINO Road-aware DLAEM + DINO-Guided Center16")
        print(f"    - Total params:     {total / 1e6:.2f} M")
        print(f"    - Trainable params: {trainable / 1e6:.2f} M")
        print(f"    - Frozen params:    {frozen / 1e6:.2f} M")
        print(f"    - DINO total/train: {dino_total / 1e6:.2f} M / {dino_train / 1e6:.2f} M")
        print("    - Data flow: X -> PEFT-DINO token bank + truncated HL_base encoder -> DLAEM8 -> layer3 -> DLAEM16 -> DINO-Guided Center16 -> decoder")
        print("    - Gradient fix: DLAEM alpha init=0.03, LightGate alpha init=0.02, DINO adapter gamma init=1e-3")
        print("    - Decoder: center16(x3',A8,A11) -> dec2(d3,x2') -> dec1(d2,x1') -> dec0(d1,x0)")
        print("-" * 60)
...
if __name__ == "__main__":
    ...
    print("RD_v2 module loaded. Instantiate through models.get_model(config['model']).")
