# -*- coding: utf-8 -*-
"""
RD_v3A: RD_v3A without auxiliary/prior supervision outputs.

本文件已经内置 RD_v2 中 RD_v3A 需要的全部 DINO / DLAEM / LightGate 模块，
以及 HL_base 中的 residual decoder 结构；部署时不再依赖 RD_v2.py 或 HL_base.py。

RD_v3A 目的：
1) 保留 RD_v3 的 PEFT-DINO + DLAEM8 + DLAEM16 + LightGate4 + HL_base decoder 主结构；
2) 删除 aux16/aux8/aux4/logit2 等辅助监督输出；
3) 不返回 prior4/prior8/prior16 logits，prior 只作为内部 soft hint；
4) 训练时只使用 final logits 计算主 BCE+Dice loss，用于诊断 RD_v3 下降是否来自辅助/先验监督干扰。

继承 RD_v3 修复：
1) DLAEM8/DLAEM16 的 alpha 默认 0.03；
2) LightGate4 的 alpha 默认 0.02；
3) DINO Adapter 的 gamma_attn/gamma_ffn 默认 1e-3。
"""
...
class RD_v3A(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(self.dino_layers) != 4:
            raise ValueError(f"RD_v3A 需要 4 个 DINO 层，例如 [2,5,8,11]，当前: {self.dino_layers}")
        ...
        # 6) RD_v3A 去掉所有辅助监督头：
        #    - 不再定义 aux16_head / aux8_head / aux4_head；
        #    - prior heads 只作为 DLAEM/LightGate 内部 soft hint，不对外返回 logits；
        #    - 训练脚本只使用 final logits 计算主 BCE+Dice loss。
    ...
    def forward_features(self, x: torch.Tensor) -> Dict[str, Any]:
        ...
        logits_half = self.out_head(d0)   # H/2, n_classes；仅内部使用，不作为辅助监督输出。
        logits = F.interpolate(logits_half, size=input_size, mode="bilinear", align_corners=False)

        # RD_v3A 只返回最终主输出，避免 train_RD 类脚本自动识别 aux/prior logits 并加入额外监督。
    ...
    def _print_model_info(self):
        ...
        print("-" * 60)
        print("[RD_v3A] RD_v3 without aux/prior supervision outputs")
        print(f"    - Total params:     {total / 1e6:.2f} M")
        print(f"    - Trainable params: {trainable / 1e6:.2f} M")
        print(f"    - Frozen params:    {frozen / 1e6:.2f} M")
        print(f"    - DINO total/train: {dino_total / 1e6:.2f} M / {dino_train / 1e6:.2f} M")
        print("    - Data flow: X -> PEFT-DINO token bank + HL_base encoder -> DLAEM8 -> layer3 -> DLAEM16 -> layer4 -> HL_base decoder -> final logits only")
        print("    - Gradient fix: DLAEM alpha init=0.03, LightGate alpha init=0.02, DINO adapter gamma init=1e-3")
        print("    - Decoder: dec3(x4,x3') -> dec2(d3,x2') -> dec1(d2,x1') -> dec0(d1,x0); no aux heads")
        print("-" * 60)
...
if __name__ == "__main__":
    ...
    print("RD_v3A module loaded. Instantiate through models.get_model(config['model']).")
