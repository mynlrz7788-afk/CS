...
class FrozenDINOv3TokenExtractor(nn.Module):
    ...
    def __init__(
        ...
        print(
            "[DC_v4_2] Frozen DINOv3 token extractor ready. "
    ...
    def _build_dino_backbone(self):
        ...
        if self.dino_ckpt_path is not None and self.dino_ckpt_path != "":
            ...
            print(
                f"[DC_v4_2] 加载 DINOv3 权重完成: {self.dino_ckpt_path}\n"
...
class DC_v4_2(nn.Module):
    """
    DC_v4_2: DC_v4_1 去掉 x4 / layer4 / dec3 的 BS4 版本。
    ...
    和 DC_v4_1 保持一致的部分：
        ...
        6. Simple residual injection 到 x3/x2/x1
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 注意：DC_v4_2 不注册 self.layer4，确保真正砍掉 x4 参数。
    ...
    def _print_trainable_summary(self):
        ...
        print("--------------------------------------------------")
        print("🔧 DC_v4_2 DINO-centric Structure Pyramid + BS4 Encoder")
        print(f"    - 总参数量:       {total / 1e6:.2f} M")
        print(f"    - 可训练参数量:   {trainable / 1e6:.2f} M")
        print(f"    - DINO原始主干可训练: {dino_trainable / 1e6:.2f} M")
        print("    - 说明: DINOv3 原始主干冻结；DINO侧 Token/A/W/G 完全保留；CNN侧砍掉 x4/layer4/dec3")
        print("--------------------------------------------------")
    ...
    def forward_features(self, x, need_aux=False):
        ...
        if input_size[0] % self.dino_patch_size != 0 or input_size[1] % self.dino_patch_size != 0:
            raise RuntimeError(
                f"DC_v4_2 要求输入 H/W 能被 dino_patch_size 整除。"
        ...
        # Simple residual injection. 和 DC_v4_1 完全一致。
