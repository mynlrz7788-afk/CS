...
class FrozenDINOv3TokenExtractor(nn.Module):
    ...
    def __init__(
        ...
        print(
            "[HLD_v2] Frozen DINOv3 token extractor ready. "
    ...
    def _build_dino_backbone(self):
        ...
        if self.dino_ckpt_path is not None and self.dino_ckpt_path != "":
            ...
            print(
                f"[HLD_v2] 加载 DINOv3 权重完成: {self.dino_ckpt_path}\n"
...
class RoadDSGDPromptExtractorG(nn.Module):
    """
    DINO-safe road prompt extractor for HLD_v2.
...
class HLDReadout(nn.Module):
    """
    DINO readout for HLD_v2.

    Key difference from HLD_v1_G:
    - G4_sem is kept clean: it is generated from T2/T5 and P8 only;
    - P4 is NOT fused into G4_sem;
    - P4 will be refined later by SCSR and then fused into G4_dec.
    """
    def __init__(
        self,
        embed_dim=384,
        ...
        c4=96,
        p8_channels=128,
        p4_channels=96,  # kept for config compatibility; not used in clean semantic readout
        gn_groups=8,
    ):
        super().__init__()
        ...
        self.g4_sem_refine = ResidualRefineGN(c4, gn_groups=gn_groups)
    ...
    def forward(self, token_dict: Dict[int, torch.Tensor], p8, patch_hw: Tuple[int, int]):
        ...
        # H/16 semantic body guidance
        g16 = self.g16_proj(self._weighted_sum(t8, t11, self.w16))

        # H/8 semantic-structure guidance
        g8_raw = self.g8_proj(self._weighted_sum(t5, t8, self.w8))
        g8 = self.g8_fuse(g8_raw, p8)

        # Clean H/4 semantic guidance.
        # It uses T2/T5 + P8 only. P4 is intentionally excluded here.
        g4_raw = self.g4_proj(self._weighted_sum(t2, t5, self.w4))
        p8_c4 = self.p8_to_c4(p8)
        g4_mid = self.g4_fuse8(g4_raw, p8_c4)  # H/8
        g4_sem = F.interpolate(g4_mid, scale_factor=2.0, mode="bilinear", align_corners=False)
        g4_sem = self.g4_sem_refine(g4_sem)

        return g16, g8, g4_sem
...
class SemanticControlledSkipRefinement(nn.Module):
    """
    SCSR: Semantic-Controlled Skip Refinement.

    It refines shallow high-resolution features using clean DINO semantic guidance G4_sem.
    Compared with a simple spatial gate, SCSR performs both channel-wise and spatial-wise
    refinement, while keeping a small residual floor to avoid excessive recall drop.
    """
    def __init__(self, shallow_ch, sem_ch, out_ch, min_gate=0.20, gn_groups=8):
        super().__init__()
        self.min_gate = float(min_gate)
        hidden = max(out_ch // 4, 16)
        spatial_hidden = max(out_ch // 2, 32)

        self.shallow_proj = ConvGNAct(shallow_ch, out_ch, kernel_size=1, padding=0, gn_groups=gn_groups)
        self.sem_proj = ConvGNAct(sem_ch, out_ch, kernel_size=1, padding=0, gn_groups=gn_groups)

        self.channel_gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(out_ch * 2, hidden, kernel_size=1, bias=True),
            nn.GELU(),
            nn.Conv2d(hidden, out_ch, kernel_size=1, bias=True),
            nn.Sigmoid(),
        )
        self.spatial_gate = nn.Sequential(
            nn.Conv2d(out_ch * 2, spatial_hidden, kernel_size=3, padding=1, bias=False),
            _make_gn(spatial_hidden, gn_groups),
            nn.GELU(),
            nn.Conv2d(spatial_hidden, 1, kernel_size=1, bias=True),
            nn.Sigmoid(),
        )
        self.refine = ResidualRefineGN(out_ch, gn_groups=gn_groups)

    def forward(self, shallow, sem):
        if sem.shape[-2:] != shallow.shape[-2:]:
            sem = F.interpolate(sem, size=shallow.shape[-2:], mode="bilinear", align_corners=False)

        s = self.shallow_proj(shallow)
        g = self.sem_proj(sem)
        fg = torch.cat([s, g], dim=1)

        c_gate = self.channel_gate(fg)
        s_gate = self.spatial_gate(fg)

        min_gate = self.min_gate
        s_safe = s * (min_gate + (1.0 - min_gate) * c_gate) * (min_gate + (1.0 - min_gate) * s_gate)
        return self.refine(s_safe + g)
...
class HeterogeneousRoadDecoderV2(nn.Module):
    """
    HLD_v2 decoder.

    Main changes:
    1) Uses x1_refined and P4_refined from SCSR instead of raw x1/P4;
    2) Uses clean G4_sem to produce detail gate, and G4_dec for decoding;
    3) Road presence gate is only-suppression: it can reduce logits, but cannot globally boost road probability.
    """
    def __init__(
        self,
        c16=256,
        ...
        self.detail_gate = nn.Sequential(
            ...
            nn.Conv2d(c4, c4, kernel_size=1, bias=True),
            nn.Sigmoid(),
        )
        ...
        if self.use_presence_gate:
            ...
            # Initialize as almost no suppression: sigmoid(5) ~= 0.993.
            nn.init.zeros_(self.presence_head[-1].weight)
            nn.init.constant_(self.presence_head[-1].bias, 5.0)
        else:
            self.presence_head = None

    def forward(self, x0, x1_refined, x2, x3, p4_refined, p8, p16, g4_sem, g4_dec, g8, g16, input_size: Tuple[int, int]):
        f16_body, f8_body, f4_body = self.body(
            x1_refined,
            x2,
            x3,
            p4_refined,
        ...
        detail_gate = self.detail_gate(g4_sem)
        f4_detail = self.detail(g4_dec, p4_refined, f4_body)
        beta = torch.clamp(self.detail_beta, 0.0, 1.0)
        f4 = self.detail_refine(f4_body + beta * detail_gate * f4_detail)
        ...
        p_exist = None
        suppression_bias = None
        if self.use_presence_gate:
            gp = torch.cat([g16, p16], dim=1)
            p_exist = torch.sigmoid(self.presence_head(gp)).view(-1, 1, 1, 1)
            suppression_bias = -self.presence_gamma * (1.0 - p_exist)
            logits = logits + suppression_bias
            logits_half = logits_half + suppression_bias
...
class HLD_v2(nn.Module):
    """
    HLD_v2: DINO-safe High-Low DINO Adaptation Network for Road Extraction.
    ...
    def __init__(
        ...
        use_presence_gate=True,
        presence_gamma=0.30,
        x1_scsr_min_gate=0.20,
        p4_scsr_min_gate=0.15,
        wpda_alpha_max=0.15,
        **kwargs,
    ):
        super().__init__()
        ...
        if 2 not in self.dino_layers:
            raise ValueError("HLD_v2 当前 readout 需要 dino_layers 包含 2。")
        ...
        self.semantic_refiner = nn.ModuleDict({
            "x1": SemanticControlledSkipRefinement(
                shallow_ch=64,
                sem_ch=decoder_c4,
                out_ch=64,
                min_gate=x1_scsr_min_gate,
                gn_groups=gn_groups,
            ),
            "p4": SemanticControlledSkipRefinement(
                shallow_ch=prompt_c4,
                sem_ch=decoder_c4,
                out_ch=prompt_c4,
                min_gate=p4_scsr_min_gate,
                gn_groups=gn_groups,
            ),
            "g4_dec_fuse": PromptFuse(decoder_c4, prompt_c4, decoder_c4, gn_groups=gn_groups),
        })

        self.decoder = HeterogeneousRoadDecoderV2(
            c16=decoder_c16,
            c8=decoder_c8,
            c4=decoder_c4,
            c2=decoder_c2,
    ...
    def _print_trainable_summary(self):
        ...
        print("--------------------------------------------------")
        print("🔧 HLD_v2 DINO-safe High-Low Adaptation Network")
        ...
        print("    - 说明: DINOv3 原始主干冻结；训练 DINO-safe Prompt、WPDA-G Adapter、HLD Readout、SCSR 和受控 detail decoder")
        print("--------------------------------------------------")
    ...
    def forward_features(self, x, need_aux=False):
        ...
        if input_size[0] % self.dino_patch_size != 0 or input_size[1] % self.dino_patch_size != 0:
            raise RuntimeError(
                f"HLD_v2 要求输入 H/W 能被 dino_patch_size 整除。当前输入={input_size}, patch_size={self.dino_patch_size}"
            )
        ...
        g16, g8, g4_sem = self.readout(adapted_tokens, p8=p8, patch_hw=patch_hw)

        x1_refined = self.semantic_refiner["x1"](x1, g4_sem)
        p4_refined = self.semantic_refiner["p4"](p4, g4_sem)
        g4_dec = self.semantic_refiner["g4_dec_fuse"](g4_sem, p4_refined)

        logits, dec_aux = self.decoder(
            x0=x0,
            x1_refined=x1_refined,
            x2=x2,
            x3=x3,
            p4_refined=p4_refined,
            p8=p8,
            p16=p16,
            g4_sem=g4_sem,
            g4_dec=g4_dec,
    ...
    def get_param_groups(self, base_lr=1e-4, weight_decay=0.01):
        ...
        named_modules = [
            ...
            (
                ...
                float(lr_cfg.get("semantic_refiner_lr", lr_cfg.get("hld_readout_lr", base_lr))),
                float(wd_cfg.get("semantic_refiner_wd", wd_cfg.get("hld_readout_wd", weight_decay))),
            ),
            (
                "decoder",
                self.decoder,
