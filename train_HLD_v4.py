...
# =========================================================
# 6.1 HLD_v4 auxiliary losses: road prior + skeleton prior
# =========================================================
...
def get_skeleton_weight(epoch: int, config: Dict[str, Any]) -> float:
    loss_cfg = config.get("training", {}).get("hld_v4_loss", {})
...
def compute_hld_v4_loss(preds, masks, criterion, epoch: int, config: Dict[str, Any]):
    """
    L_total = L_final + w_half*L_half + w_prior*L_prior + w_skel*L_skeleton
    如果模型不是 HLD_v4 dict 输出，则自动退化成普通 final loss。
    """
    loss_cfg = config.get("training", {}).get("hld_v4_loss", {})
...
def main():
    ...
    model_cfg_msg = (
        ...
        f" - HLD_v4 loss: {config.get('training', {}).get('hld_v4_loss', {})}\n"
        f"--------------------------------------------------\n"
    )
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for batch_data in train_loader_tqdm:
            ...
            with torch.amp.autocast("cuda"):
                ...
                loss, loss_items = compute_hld_v4_loss(model_out, masks, criterion, epoch=epoch, config=config)
